...
For convenience, there are three GISDK functions which have been implemented
directly in R and aren't called using `RunFunction()`:
...
is are also made available. These can be passed into `RunFunction()` and
...
```{r}
RunFunction("GetMatrixBaseIndex", matrix$ref)
