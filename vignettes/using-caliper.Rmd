...
If you call the `ShowArray()` function or a macro that contains `ShowArray()`, R
will wait until you have closed that pop up before continuing. You can test the
behavior by running the code below. You may have to alt-tab to find the pop up
window.
...
This same behavior can be observed for other functions like `ShowMessage()`,
but only if connected to a program that is already open (i.e. that has a 
visible window).
