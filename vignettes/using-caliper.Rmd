...
Several generic functions can be used on `matrix`.
...
```{r}
mtx <- as.matrix(matrix, core = "Travel Time")
mtx[1:5, 1:5]
```

For more advanced use, COM pointers to the matrix handle and each currency
are also made available. These can be passed into `RunFunction()` and
...
RunFunction("GetMatrixColumnLabels", matrix$cores$`In-Vehicle Time`)
```
...
access object fields (attributes) and methods. The code below sets the `Label`
...
The code block below gives a snippet of several data structures returned by
GISDK and what they look like in R. Most results make sense.

  * Arrays and vectors are returned as vectors
  * Named/Options arrays are returned as named lists
  
```{r}
# GISDK: return({1, 2})
RunMacro("return array")

# GISDK: return(A2V({1, 2}))
RunMacro("return vector")

# GISDK: return({{"one", 1}, {"two", 2}})
...
However, if all elements of a nested GISDK array are the same type (e.g.
numeric), the array is flattened.

```{r}
# GISDK: return({{1, 2}, {3, 4}})
RunMacro("return nested array")
```

This is a rare case and can be dealt with in your R code. For example, if you
know what the structure should be (as in our example), you can use `relist()`.

```{r}
x <- RunMacro("return nested array")
relist(x, list(1:2, 1:2))
```
