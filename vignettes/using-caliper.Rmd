...
directly in the `caliper` package:
  ...
  * `CreateObject()` (`vignette("caliper-objects")`)
...
```{r, include=FALSE}
v_table <- vignette(package = "caliper")$results[, 3]
v_table <- v_table[v_table != "using-caliper"]
v_table <- data.frame(Vignettes = v_table)
```
...
For example, learn about working with matrices using
`vignette("caliper-matrices")`.
