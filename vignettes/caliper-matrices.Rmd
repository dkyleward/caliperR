...
`caliper` package. Many of the relevant GISDK functions have been streamlined
...
package. If not, see `vignette("using-caliper")`.
...
You can also send updated data back to Caliper software. In the code below,
...
or currency was. The `caliper` package handles this behind the scenes, but some
...
object. `RunFunction` will understand what you mean.

```{r}
# A GISDK function requiring a matrix handle
RunFunction("GetMatrixBaseIndex", mtx)
```

The function `GetMatrixColumnLabels()` requires a matrix currency. Simply
reference the core by name and `RunFunction` handles the rest. Note that the
